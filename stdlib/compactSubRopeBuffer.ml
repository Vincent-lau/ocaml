...
let create _ = ref CompactSubRope.empty

let contents b = CompactSubRope.to_string !b

(* convert the rope to bytes
and sub it *)
let to_bytes b = 
  let s = CompactSubRope.to_string !b in 
  let bytes = Bytes.of_string s in
  Bytes.sub bytes 0 (CompactSubRope.length !b)

let sub b ofs len =
  if ofs < 0 || len < 0 || ofs > (CompactSubRope.length !b) - len
  then invalid_arg "CompactSubRopeBuffer.sub"
  else CompactSubRope.to_string (CompactSubRope.sub !b ofs len)


let blit src srcoff dst dstoff len =
  if len < 0 || srcoff < 0 || srcoff > (CompactSubRope.length !src) - len
             || dstoff < 0 || dstoff > (Bytes.length dst) - len
  then invalid_arg "CompactSubRopeBuffer.blit"
  else
    CompactSubRope.blit !src srcoff dst dstoff len


let nth b ofs =
  if ofs < 0 || ofs >= (CompactSubRope.length !b) then
   invalid_arg "CompactSubRopeBuffer.nth"
  else CompactSubRope.get !b ofs


let length b = CompactSubRope.length !b


let reset b = b := CompactSubRope.empty

(* if rope is used, it makes sense that clear and reset are the same *)
let clear = reset


(* this add_char will use CompactSubRope.(^) to add a char to the buffer/rope *)
let add_char b c =
  let new_r = CompactSubRope.make 1 c in
  b := (CompactSubRope.(^) !b new_r)
...
let add_substring b s offset len =
  ...
  then invalid_arg "CompactSubRopeBuffer.add_substring/add_subbytes";
  let sub_r = CompactSubRope.of_string (String.sub s offset len) in
  b := (CompactSubRope.(^) !b sub_r)
...
let add_string b s =
  let r = CompactSubRope.of_string s in
  b := (CompactSubRope.(^) !b r)
...
let really_input_up_to ic buf ofs len =
  let rec loop ic buf ~already_read ~ofs ~to_read =
    if to_read = 0 then already_read
      ...
      let rope = CompactSubRope.of_string (Bytes.to_string (Bytes.sub b 0 r)) in
      buf := (CompactSubRope.(^) !buf rope);
...
let add_channel b ic len =
  if len < 0 || len > Sys.max_string_length then   (* PR#5004 *)
    invalid_arg "CompactSubRopeBuffer.add_channel";
...
let truncate b len =
    if len < 0 || len > length b then
      invalid_arg "CompactSubRopeBuffer.truncate"
    else
      b := (CompactSubRope.sub !b 0 len)
...
let add_int8 b x =
  ...
  let rope = CompactSubRope.of_string (Bytes.to_string byte) in
  b := CompactSubRope.(^) !b rope
