(* In this implementation each buffer stores a CompactRope rather than bytes/string
decrease implementation complexity*)

type t = CompactRope.t ref


(* create n does not make much sense here because it is better
to keep, e.g. small CompactRope optimisation in the CompactRope module 
therefore for a CompactRope buffer, it is always true that b.position = b.length *)
let create _ = ref CompactRope.empty

let contents b = CompactRope.to_string !b

(* convert the CompactRope to bytes
and sub it *)
let to_bytes b = 
  let s = CompactRope.to_string !b in 
  let bytes = Bytes.of_string s in
  Bytes.sub bytes 0 (CompactRope.length !b)

let sub b ofs len =
  if ofs < 0 || len < 0 || ofs > (CompactRope.length !b) - len
  then invalid_arg "CompactRopeBuffer.sub"
  else CompactRope.to_string (CompactRope.sub !b ofs len)


let blit src srcoff dst dstoff len =
  if len < 0 || srcoff < 0 || srcoff > (CompactRope.length !src) - len
             || dstoff < 0 || dstoff > (Bytes.length dst) - len
  then invalid_arg "CompactRopeBuffer.blit"
  else
    CompactRope.blit !src srcoff dst dstoff len


let nth b ofs =
  if ofs < 0 || ofs >= (CompactRope.length !b) then
   invalid_arg "CompactRopeBuffer.nth"
  else CompactRope.get !b ofs


let length b = CompactRope.length !b


let reset b = b := CompactRope.empty

(* if CompactRope is used, it makes sense that clear and reset are the same *)
let clear = reset


(* this add_char will use CompactRope.(^) to add a char to the buffer/CompactRope *)
let add_char b c =
  let new_r = CompactRope.make 1 c in
  b := (CompactRope.(^) !b new_r)
...
let add_substring b s offset len =
  ...
  then invalid_arg "CompactRopeBuffer.add_substring/add_subbytes";
  let sub_r = CompactRope.of_string (String.sub s offset len) in
  b := (CompactRope.(^) !b sub_r)
...
let add_string b s =
  let r = CompactRope.of_string s in
  b := (CompactRope.(^) !b r)
...
let really_input_up_to ic buf ofs len =
  let rec loop ic buf ~already_read ~ofs ~to_read =
    if to_read = 0 then already_read
      ...
      let rope = CompactRope.of_string (Bytes.to_string (Bytes.sub b 0 r)) in
      buf := (CompactRope.(^) !buf rope);
...
let add_channel b ic len =
  if len < 0 || len > Sys.max_string_length then   (* PR#5004 *)
    invalid_arg "CompactRopeBuffer.add_channel";
...
let truncate b len =
    if len < 0 || len > length b then
      invalid_arg "CompactRopeBuffer.truncate"
    else
      b := (CompactRope.sub !b 0 len)
...
let add_int8 b x =
  ...
  let rope = CompactRope.of_string (Bytes.to_string byte) in
  b := CompactRope.(^) !b rope
