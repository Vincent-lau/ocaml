...
let create _ = ref Rope.empty

let contents b = Rope.to_string !b

(* convert the rope to bytes
and sub it *)
let to_bytes b = 
  let s = Rope.to_string !b in 
  let bytes = Bytes.of_string s in
  Bytes.sub bytes 0 (Rope.length !b)

let sub b ofs len =
  if ofs < 0 || len < 0 || ofs > (Rope.length !b) - len
  then invalid_arg "RopeBuffer.sub"
  else Rope.to_string (Rope.sub !b ofs len)


let blit src srcoff dst dstoff len =
  if len < 0 || srcoff < 0 || srcoff > (Rope.length !src) - len
             || dstoff < 0 || dstoff > (Bytes.length dst) - len
  then invalid_arg "RopeBuffer.blit"
  else
    Rope.blit !src srcoff dst dstoff len


let nth b ofs =
  if ofs < 0 || ofs >= (Rope.length !b) then
   invalid_arg "RopeBuffer.nth"
  else Rope.get !b ofs


let length b = Rope.length !b


let reset b = b := Rope.empty

(* if rope is used, it makes sense that clear and reset are the same *)
let clear = reset


(* this add_char will use Rope.(^) to add a char to the buffer/rope *)
let add_char b c =
  let new_r = Rope.make 1 c in
  b := (Rope.(^) !b new_r)
...
let add_substring b s offset len =
  ...
  then invalid_arg "RopeBuffer.add_substring/add_subbytes";
  let sub_r = Rope.of_string (String.sub s offset len) in
  b := (Rope.(^) !b sub_r)
...
let really_input_up_to ic buf ofs len =
  let rec loop ic buf ~already_read ~ofs ~to_read =
    if to_read = 0 then already_read
      ...
      let rope = Rope.of_string (Bytes.to_string (Bytes.sub b 0 r)) in
      buf := (Rope.(^) !buf rope);
...
let add_channel b ic len =
  if len < 0 || len > Sys.max_string_length then   (* PR#5004 *)
    invalid_arg "RopeBuffer.add_channel";
...
let truncate b len =
    if len < 0 || len > length b then
      invalid_arg "RopeBuffer.truncate"
    else
      b := (Rope.sub !b 0 len)
...
let add_int8 b x =
  ...
  let rope = Rope.of_string (Bytes.to_string byte) in
  b := Rope.(^) !b rope
