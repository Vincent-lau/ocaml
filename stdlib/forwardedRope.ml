(* this module should be the same as the CompactRope
module but uses native pattern matching instead of 
the destruct function
May need to consider delete this module and just implement
the CompactRope module use this approach *)
...
type t =
  ...
  | Branch: {leftlen: int; left: t; right: t} -> t [@forward]
type compactRope = t

external get_rope_fwd_fun: unit -> t fwd = "caml_get_rope_fwd"

let rope_fwd_fun = get_rope_fwd_fun ()
...
let rope_concat r1 r2 = 
  if is_empty r1 then r2 
    ...
    {_fwd_fun = rope_fwd_fun; leftlen = (length r1); left = r1; right = r2}
...
let rec map f = function
  ...
  | Branch {left; right} -> map f left ^ map f right
...
let rec mapi_rec f ofs r = 
  ...
  | Branch {leftlen; left; right} ->
    let left = mapi_rec f ofs left 
    and right = mapi_rec f (ofs + leftlen) right in
    left ^ right
...
let rec escaped = function
  ...
  | Branch {left; right} ->
    let left = escaped left and 
    right = escaped right in 
    left ^ right
