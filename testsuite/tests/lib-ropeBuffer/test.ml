...
let buf = RopeBuffer.create n
;;

let () =
  for i = 1 to 10 do
    RopeBuffer.add_char buf 'a'
  done
;;

assert (RopeBuffer.length buf = n)
;;

(* Helpers *)

let output result str =
  print_string ("RopeBuffer " ^ str ^ " " ^ result ^ "\n")
;;
...
let truncate_correct : unit =
  ...
  try
    RopeBuffer.truncate buf n';
    if RopeBuffer.length buf = n' then
      passed msg
    else
      failed msg
  with
...
let reset_non_zero : unit =
  ...
  RopeBuffer.reset buf;
  if RopeBuffer.length buf = 0 then
    passed msg
  else
    failed msg
;;
...
let test_spec_map msg utf_x_map ropeBuffer_add_utf_x_uchar =
  let b = RopeBuffer.create 4 in
  let rec loop u =
    RopeBuffer.clear b; ropeBuffer_add_utf_x_uchar b u;
    match RopeBuffer.contents b = utf_x_map.(Uchar.to_int u) with
...
let add_utf_8_uchar : unit =
  ...
  test_spec_map
    "add_utf_8_uchar: test against spec" map RopeBuffer.add_utf_8_uchar
;;

let add_utf_16be_uchar : unit =
  let map = uchar_map_of_spec utf_16be_spec in
  test_spec_map
    "add_utf_16be_uchar: test against spec" map RopeBuffer.add_utf_16be_uchar
;;
...
let add_utf_16le_uchar : unit =
  ...
  in
  test_spec_map
    "add_utf_16le_uchar: test against spec" map RopeBuffer.add_utf_16le_uchar
;;
...
let () =
  let b = RopeBuffer.create 64 in
  RopeBuffer.add_int8 b 0xff;
  RopeBuffer.add_int8 b 0x01;
  RopeBuffer.add_int16_be b 0x0123;
  RopeBuffer.add_int16_le b 0x0123;
  RopeBuffer.add_int32_be b 0x01234567l;
  RopeBuffer.add_int32_le b 0x01234567l;
  RopeBuffer.add_int64_be b 0x0123456789abcdefL;
  RopeBuffer.add_int64_le b 0x0123456789abcdefL;
  assert (RopeBuffer.contents b =
          "\xff\x01" ^
          "\x01\x23\x23\x01" ^
          "\x01\x23\x45\x67" ^
          "\x67\x45\x23\x01" ^
  ...
  RopeBuffer.clear b;
  RopeBuffer.add_int16_ne b 0x0123;
  RopeBuffer.add_int32_ne b 0x01234567l;
  RopeBuffer.add_int64_ne b 0x0123456789abcdefL;
  let s = RopeBuffer.contents b in
  ...
  for i = 1 to 20 do
    let b = RopeBuffer.create i in
    for j = 1 to 100 do
      RopeBuffer.add_int8 b 1
    done;
    assert(RopeBuffer.length b = 100);
  done;
  for i = 1 to 20 do
    let b = RopeBuffer.create i in
    for j = 1 to 100 do
      RopeBuffer.add_int16_ne b 1
    done;
    assert(RopeBuffer.length b = 200);
  done;
  for i = 1 to 20 do
    let b = RopeBuffer.create i in
    for j = 1 to 100 do
      RopeBuffer.add_int32_ne b 1l
    done;
    assert(RopeBuffer.length b = 400);
  done;
  for i = 1 to 20 do
    let b = RopeBuffer.create i in
    for j = 1 to 100 do
      RopeBuffer.add_int64_ne b 1L
    done;
    assert(RopeBuffer.length b = 800);
  done
;;
