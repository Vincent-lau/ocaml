...
let buf = CompactRopeBuffer.create n
;;

let () =
  for i = 1 to 10 do
    CompactRopeBuffer.add_char buf 'a'
  done
;;

assert (CompactRopeBuffer.length buf = n)
;;

(* Helpers *)

let output result str =
  print_string ("CompactRopeBuffer " ^ str ^ " " ^ result ^ "\n")
;;
...
let compactRopeBuffer_truncate = "CompactRopeBuffer.truncate"

let unexpected str =
  Printf.sprintf "The Invalid_argument exception has been raised with an \
    invalid value as argument \"%s\". Expecting \"%s\"."
    str compactRopeBuffer_truncate

let validate f str msg =
  if str=compactRopeBuffer_truncate then f msg
  else failed (unexpected str)

(* Tests *)
let () = print_string "Standard Library: Module CompactRopeBuffer\n"
;;

let truncate_neg : unit =
  let msg =  "truncate: negative" in
  try
    CompactRopeBuffer.truncate buf (-1);
    failed msg
  with
...
let truncate_large : unit =
  ...
  try
    CompactRopeBuffer.truncate buf (n+1);
    failed msg
  with
    Invalid_argument str -> validate passed str msg
;;
...
let truncate_correct : unit =
  ...
  try
    CompactRopeBuffer.truncate buf n';
    if CompactRopeBuffer.length buf = n' then
      passed msg
    else
      failed msg
  with
...
let reset_non_zero : unit =
  ...
  CompactRopeBuffer.reset buf;
  if CompactRopeBuffer.length buf = 0 then
    passed msg
  else
    failed msg
;;
...
let test_spec_map msg utf_x_map compactRopeBuffer_add_utf_x_uchar =
  let b = CompactRopeBuffer.create 4 in
  let rec loop u =
    CompactRopeBuffer.clear b; compactRopeBuffer_add_utf_x_uchar b u;
    match CompactRopeBuffer.contents b = utf_x_map.(Uchar.to_int u) with
...
let add_utf_8_uchar : unit =
  ...
  test_spec_map
    "add_utf_8_uchar: test against spec" map CompactRopeBuffer.add_utf_8_uchar
;;

let add_utf_16be_uchar : unit =
  let map = uchar_map_of_spec utf_16be_spec in
  test_spec_map
    "add_utf_16be_uchar: test against spec" map CompactRopeBuffer.add_utf_16be_uchar
;;
...
let add_utf_16le_uchar : unit =
  ...
  in
  test_spec_map
    "add_utf_16le_uchar: test against spec" map CompactRopeBuffer.add_utf_16le_uchar
;;
...
let () =
  let b = CompactRopeBuffer.create 64 in
  CompactRopeBuffer.add_int8 b 0xff;
  CompactRopeBuffer.add_int8 b 0x01;
  CompactRopeBuffer.add_int16_be b 0x0123;
  CompactRopeBuffer.add_int16_le b 0x0123;
  CompactRopeBuffer.add_int32_be b 0x01234567l;
  CompactRopeBuffer.add_int32_le b 0x01234567l;
  CompactRopeBuffer.add_int64_be b 0x0123456789abcdefL;
  CompactRopeBuffer.add_int64_le b 0x0123456789abcdefL;
  assert (CompactRopeBuffer.contents b =
          "\xff\x01" ^
          "\x01\x23\x23\x01" ^
          "\x01\x23\x45\x67" ^
          "\x67\x45\x23\x01" ^
  ...
  CompactRopeBuffer.clear b;
  CompactRopeBuffer.add_int16_ne b 0x0123;
  CompactRopeBuffer.add_int32_ne b 0x01234567l;
  CompactRopeBuffer.add_int64_ne b 0x0123456789abcdefL;
  let s = CompactRopeBuffer.contents b in
  ...
  for i = 1 to 20 do
    let b = CompactRopeBuffer.create i in
    for j = 1 to 100 do
      CompactRopeBuffer.add_int8 b 1
    done;
    assert(CompactRopeBuffer.length b = 100);
  done;
  for i = 1 to 20 do
    let b = CompactRopeBuffer.create i in
    for j = 1 to 100 do
      CompactRopeBuffer.add_int16_ne b 1
    done;
    assert(CompactRopeBuffer.length b = 200);
  done;
  for i = 1 to 20 do
    let b = CompactRopeBuffer.create i in
    for j = 1 to 100 do
      CompactRopeBuffer.add_int32_ne b 1l
    done;
    assert(CompactRopeBuffer.length b = 400);
  done;
  for i = 1 to 20 do
    let b = CompactRopeBuffer.create i in
    for j = 1 to 100 do
      CompactRopeBuffer.add_int64_ne b 1L
    done;
    assert(CompactRopeBuffer.length b = 800);
  done
;;
